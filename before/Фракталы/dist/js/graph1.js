{
	...
	function fillFract1(sizeOfIteration,Z,curSize,SWC,DB,distBetwen,ref,curmod,kantorData) {
		...
		if(kantorData['v'+curmod].fill.length > 0) {
			...
			let p = grafar.constant(0).select();
	...
	let ref = grafar.constant(0).select()

	let DB = grafar.constant(sizeOfIteration).select()
	...
	let SI = grafar.constant(sizeOfIteration).select()
	...
	let Z = grafar.constant(-sizeOfIteration).select()

	let SWC = grafar.constant(curSize/3).select()
	...
	function kantor(x,y,z,ZC,sw,SWC,sh,SHC,sd,i,ci,ref,p,t1,t2,mod = 1) {
		...
		function recKantor1(x,y,z,ZC,sw,SWC,sh,SHC,sd,i,ci,ref,p,t1,t2) {
			let r = grafar.constant(0 / 255).select()
			let g = grafar.constant(140 / 255).select()
			let b = grafar.constant(240 / 255).select()
			...
			let Z = grafar.constant(z+sh+sd).select()
			...
			if (i<ci){
				...
				let SWC = grafar.constant(sw/3).select()
		...
		function recKantor3(x,y,z,ZC,sw,SWC,sh,SHC,sd,i,ci,ref,p,t1,t2) {
			...
			let Z = grafar.constant(z+sh).select()

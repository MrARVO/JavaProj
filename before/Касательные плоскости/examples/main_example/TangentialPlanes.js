(function() {
		...
		function planePoints(points) {
			var eigVec_1 = vecSub(points._2, points._1),
				eigVec_2 = vecSub(points._3, points._1);
		...
		function updateProblem() {
			...
			if (autoPtSelect) {
				points.point_1[0] = Main_point[0] + 3.5;
				points.point_1[1] = Main_point[1];
				points.point_2[0] = Main_point[0];
				points.point_2[1] = Main_point[1] + 3.5;
			}
			else {
			...
			points.point_1[2] = problem.eqn_comp(points.point_1[0], points.point_1[1]);
			points.point_2[2] = problem.eqn_comp(points.point_2[0], points.point_2[1]);
			...
			main_graf.reset()
					.constrain({what: 'x', maxlen: 120, as: grafar.seq(-8.5, 8.5, 'x')})
					.constrain({what: 'y', maxlen: 120, as: grafar.seq(-8.5, 8.5, 'y')})
			...
			tang_plane.reset()
						.constrain({what: 't', maxlen: 30, as: grafar.seq(-5.2, 5.2, 't')})
						.constrain({what: 's', maxlen: 30, as: grafar.seq(-5.2, 5.2, 's')})
						.constrain({what: 'x, y, z', using: 't, s', as: function(data, l) {
							var t = data.t, s = data.s;
							for (var i = 0; i < l; i++) {
								data.x[i] = Main_point[0] + t[i] * xVec[0] + s[i] * yVec[0];
								data.y[i] = Main_point[1] + t[i] * xVec[1] + s[i] * yVec[1];
								data.z[i] = Main_point[2] + t[i] * xVec[2] + s[i] * yVec[2];
							}
						}})
						.refresh();
